...
@plugindesc Fixed player position at the start of the game Ver1.1.1 (2025/5/29)
@author PotatoDragon
@license MIT License
...
## Overview
Fixes the player's position at the start of a production game.

## Usage
This plugin references the initial position only during production.

This plugin prevents the game from starting at the final boss if the initial
position is changed during test play.

1. Set the map coordinates in the plugin parameters.
2. Perform a test play and check that the map's initial position is correct.
3. Change the plugin parameter "PlayProd" to "Enable (true)."

This allows you to play from the initial position specified in the map during
test play, and from the initial position specified by the plugin when
released.

@param PlayProd
@text Only valid during production
@desc Enable it only during production
@type boolean
@on Enable
@off Always enabled
@default false
...
@param map
@text Initial Map
@desc Initial Map Information
@type location
@default {"mapId":"0","x":"0","y":"0"}

@param StartDirection
@text For initial players
@desc Player's initial orientation
@type select
@default 2
@option under
@value 2
