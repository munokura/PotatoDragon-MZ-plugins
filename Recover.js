...
@plugindesc Full recovery Ver1.0.9 (2025/1/24)
@author PotatoDragon
@license MIT License
...
## Overview
Provides a feature for full recovery in various situations.

## How to Use
The following features are provided.
Change the parameters for the feature you want to use.

1. Full Recovery (Start of Battle)
2. Full Recovery (End of Battle)
3. Full Recovery (Level Up)
4. Full Recovery (Change Job)
5. Full Recovery Command: TP Recovery

The target of recovery is (HP, MP, TP, and State).
These can be changed in the settings.
You can also optionally change whether or not to full recover with the switch.

TP is only recovered if the TP Carryover feature is enabled.
*If TP Carryover is not enabled, recovery will occur but the TP will be reset.

@param ExceptClearStates
@text Cancellation exclusion state
@desc States to exclude from state cancellation
@type state[]
@default []

@param BattleStartRecover
@text Full recovery (start of battle)
@desc Fully healed at the start of battle
@type boolean
@on recover
@off No recovery
@default false
...
@text Full recovery (start of battle) enable switch
@desc When this switch is ON, full recovery (start of battle) is enabled.
@type switch
@default 0
...
@text TP full recovery
@desc Fully recover TP
@type boolean
@on recover
@off No recovery
@default true
...
@text State cancellation possibility
@desc Setting whether to cancel the state
@type boolean
@on unlock
@off Do not release
@default true
...
@text Full recovery (end of battle)
@desc Will you be fully recovered at the end of the battle?
@type boolean
@on recover
@off No recovery
@default false
...
@text Full recovery (end of battle) enable switch
@desc When this switch is ON, full recovery (end of battle) is enabled.
@type switch
@default 0
...
@text Full recovery (level up)
@desc Will it fully recover when you level up?
@type boolean
@on recover
@off No recovery
@default false
...
@text Full recovery (level up) enable switch
@desc When this switch is ON, full recovery (level up) is enabled.
@type switch
@default 0
...
@text Full recovery (job change)
@desc Will it be fully restored when changing jobs?
@type boolean
@on recover
@off No recovery
@default false
...
@text Full recovery (job change) enable switch
@desc When this switch is ON, full recovery (job change) is enabled.
@type switch
@default 0
...
@param TpRecover
@text Full recovery command TP recovery
@desc Does the event command [Full Recovery] also recover TP?
@type boolean
@on recover
@off No recovery
@default false
...
@text Full recovery command TP recovery enable switch
@desc When this switch is ON, the full recovery command TP recovery is enabled.
@type switch
@default 0
@parent TpRecover

@param NuunFormationMember
@text All standby members recovered
@desc Will it be fully restored when NUUN_SceneFormation is installed?
@type boolean
@on recover
@off No recovery
@default true
